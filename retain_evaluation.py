...
def output_results(info_dict, y_prob, output, inflection_point):
    if output:     
        df = pd.DataFrame({'Actual': info_dict["targets"], 'Predicted_Rounded': np.where(y_prob>inflection_point, 1, 0), 'PredictedNotRounded': y_prob})
        
        df['target'] = np.where(df['Actual']==df['Predicted_Rounded'], 1, 0)
        if "pids" in info_dict:
            df["IDs"] = info_dict["pids"]
        #df["to_event"] = info_dict["to_event"]
